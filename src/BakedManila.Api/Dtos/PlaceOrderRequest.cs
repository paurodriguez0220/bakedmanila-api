...
public sealed record PlaceOrderRequest(
    [Required, MinLength(1), MaxLength(50)] List<PlaceOrderItemRequest> Items,
...
{
    // Value-type members above are nullable purely so [Required] can reject a missing
    // JSON property. ToCommand() only runs after model validation passes, so .Value is safe here.
    public NewOrder ToCommand() => new(
        Items.Select(i => new NewOrderItem(i.ProductSlug, i.Quantity)).ToList(),
        CustomerName, PhoneNumbers.NormalizePh(Phone), Email, MessengerHandle, PreferredDate!.Value,
        IsRush, Notes, FulfillmentType!.Value, PaymentMethod!.Value);
}
